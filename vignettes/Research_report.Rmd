...
get_results <- function(z) {
  d <- TOut_design_bin(rho_0 = 0.5, rho_1 = 0.7, alpha_nom = 0.05, beta_nom = 0.2, gamma = 1, eta = z)
...
ggsave("../paper/figures/eta_ns.pdf", height=4.5, width=14, units="cm")
ggsave("../paper/figures/eta_ns.eps", height=4.5, width=14, units="cm", device = cairo_ps())
```
...
The intuition here is that when we have $\tau \neq 0$, the point at which we need to control type II errors shifts left but one of the type I error components stays put at $\rho_0$.
...
```{r}
df <- expand.grid(l = seq(0, 0.1, 0.01),
                 w = seq(0, 0.05, 0.01))

ns <- apply(df, 1, function(z) TOut_design_bin(rho_0 = 0.5, rho_1 = 0.7, alpha_nom = 0.05, beta_nom = 0.2, tau = c(z[1], z[1] + z[2]), max_n = 500, eta = 0.5)$n)
df$n <- ns

ggplot(df, aes(l, w)) + geom_tile(aes(fill = n)) +
  scale_fill_continuous(type = "viridis") +
  xlab("Lower bound of adjustment effect") +
  ylab("Width of adjustment effect interval") +
  theme_minimal()
```

```{r}
df <- expand.grid(l = seq(0, 0.1, 0.001),
                 w = seq(0, 0.05, 0.001))

ns <- apply(df, 1, function(z) TOut_design_bin(rho_0 = 0.5, rho_1 = 0.7, alpha_nom = 0.05, beta_nom = 0.2, tau = c(z[1], z[1] + z[2]), max_n = 500, eta = 0.2)$n)
df$n <- ns
...
```{r}
df <- data.frame(eta = seq(0.0501, 0.5, 0.001))
ns <- sapply(df$eta, function(z) TOut_design_bin(rho_0 = 0.5, rho_1 = 0.7, alpha_nom = 0.05, beta_nom = 0.2, gamma = 1, eta = z, tau = c(0.05, 0.15), max_n = 500)$n)
df$n <- ns
...
```{r}
df <- data.frame(eta = seq(0.0501, 0.5, 0.001))
ns <- sapply(df$eta, function(z) TOut_design_cont(rho_0 = 0.5, rho_1 = 0.8, sigma = 1, alpha_nom = 0.05, beta_nom = 0.2, gamma = 1, eta = z, tau = c(0.05, 0.15), max_n = 500)$n)
df$n <- ns
