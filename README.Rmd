...
The function `opt_pc` returns a vector of the form $(n, x_0, x_1)$. In this case we find that a pilot trial with $n = 200$ patients in the intervention arm will satisfy our constraints $\alpha < 0.05, \beta < 0.2, \gamma < 0.5$ if we $stop$ when we observe 109 patients or fewer adhering, $go$ if we observe more that 131 patients adhering, and $pause$ otherwise.
...
Here we find that our earlier choice of $n = 200$ was unnecessarily large and can be reduced down to $n = 85$, with progression criteria $x_0 = 48$ and $x_1 = 55$.
