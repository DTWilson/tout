...
```{r}
tout_design(rho_0 = 0.5, rho_1 = 0.7, alpha_nom = 0.05, beta_nom = 0.1, 
            gamma_nom = 1, eta_0 = 0.5)
```
...
get_results <- function(eta) {
  # For any given eta, find the optimal design and output the sample size and 
  # thresholds
  d <- tout_design(rho_0 = 0.5, rho_1 = 0.7, alpha_nom = 0.05, beta_nom = 0.1, 
                   gamma_nom = 1, eta_0 = eta)
...
ggplot(df, aes(eta)) + geom_line(aes(y=n)) +
  ...
  xlab(expression(paste("Probability of incorrect decision following a 
                        pause outcome, ", eta))) +
  ylab("Sample size") +
  theme_minimal()
...
get_true_ocs <- function(z) {
  ...
  d <- tout_design(rho_0 = 0.5, rho_1 = 0.7, alpha_nom = 0.05, beta_nom = 0.1, 
                   gamma_nom = 1, eta_0 = 0.5, n = z[1], x = c(z[2], z[3]))
  c(d$alpha, d$beta)
}
...
ggplot(df, aes(eta)) + 
  ...
  xlab(expression(paste("Assumed probability of incorrect decision following 
                        a pause outcome, ", eta))) +
  ylab("Error rate") + 
...
ggplot(df, aes(gamma)) + geom_line(aes(y=n)) +
  geom_line(aes(y = dif), linetype=2) + 
  xlab(expression(paste("Maximum probability of incorrect conclusive 
                        decision, ", gamma, "*"))) +
  ylab("Sample size") +
  theme_minimal()
...
ns <- sapply(df$tau, function(z) tout_design(rho_0 = 0.5, rho_1 = 0.7, 
                                             alpha_nom = 0.05, beta_nom = 0.1, 
                                             tau = c(z, z), max_n = 500)$n)
...
```{r}
# Create a grid on adjustment effect interval lower bounds (l) and widths (w)
df <- expand.grid(l = seq(0, 0.1, 0.005),
                 w = seq(0, 0.05, 0.005))

ns <- apply(df, 1, function(z) tout_design(rho_0 = 0.5, rho_1 = 0.7, 
                                           alpha_nom = 0.05, beta_nom = 0.1,
                                           tau = c(z[1], z[1] + z[2]), 
                                           max_n = 700)$n)
df$n <- ns
...
ggplot(df, aes(l, w)) + geom_tile(aes(fill = n)) +
  ...
  ylab(expression(paste("Width of adjustment effect interval, ",
                        tau[max] - tau[min]))) +
  theme_minimal()
...
tout_design(rho_0 = 0.5, rho_1 = 0.7, alpha_nom = 0.05, beta_nom = 0.1, 
            tau = c(0.1, 0.15), max_n = 8000)$n
tout_design(rho_0 = 0.5, rho_1 = 0.7, alpha_nom = 0.05, beta_nom = 0.1, 
            tau = c(0.0, 0.15), max_n = 8000)$n

# Change lower bound from 0.05 to 0.01 while upper bound remains 0.05
tout_design(rho_0 = 0.5, rho_1 = 0.7, alpha_nom = 0.05, beta_nom = 0.1, 
            tau = c(0.05, 0.05), max_n = 8000)$n
tout_design(rho_0 = 0.5, rho_1 = 0.7, alpha_nom = 0.05, beta_nom = 0.1, 
            tau = c(0.01, 0.05), max_n = 8000)$n
```
...
ns <- sapply(df$eta, function(z) tout_design(rho_0 = 0.5, rho_1 = 0.7, 
                                             alpha_nom = 0.05, beta_nom = 0.1, 
                                             eta_0 = z, tau = c(0.05, 0.1),
                                             max_n = 500)$n)
df$n <- ns

ggplot(df, aes(eta)) + geom_line(aes(y=n)) +
  xlab(expression(paste("Probability of incorrect decision following 
                        a pause outcome, ", eta))) +
  ylab("Sample size") +
  theme_minimal()
```
...
### Relaxing error rate constraints while fixing $n$
...
ggplot(df, aes(alpha, beta)) + geom_point(aes(colour = gamma)) +
  geom_point(data = df[df$x_0 == 19 & df$x_1 == 19,], shape = 2, 
             size = 5, colour = "blue") +
  geom_point(data = df[df$x_0 == 17 & df$x_1 == 17,], shape = 2, 
             size = 5, colour = "green") +
  geom_point(data = df[df$x_0 == 15 & df$x_1 == 20,], shape = 2, 
             size = 5, colour = "red") +
...
ggplot(data.frame(x = c(-10, 10)), aes(x)) +
  ...
  stat_function(fun = dnorm, geom = "area", fill = "orange", alpha = 0.4, 
                xlim = c(2, 7)) +
  stat_function(fun = dnorm, geom = "area", fill = "red", alpha = 0.4, 
                xlim = c(1, 2)) +
  
  # Alternative
  stat_function(fun = dnorm, args = list(mean = 2.5), geom = "line", 
                aes(linetype="b")) +
  stat_function(fun = dnorm, args = list(mean = 2.5), 
                geom = "area", fill = "darkgreen", alpha = 0.4, 
                xlim = c(-3, 1)) +
  stat_function(fun = dnorm, args = list(mean = 2.5), 
                geom = "area", fill = "blue", alpha = 0.4, xlim = c(1, 2)) +
  
  geom_label(aes(x = 0.6, y = 0.22, label = "lambda"), parse = T, 
             fill = "red", alpha = 0.4) +
  geom_label(aes(x = 2.4, y = 0.17, label = "delta"), parse = T, 
             fill = "blue", alpha = 0.4) +
  geom_label(aes(x = 0.2, y = 0.1, label = "beta[a]"), parse = T, 
             fill = "darkgreen", alpha = 0.4) +
  geom_label(aes(x = 2.6, y = 0.06, label = "alpha[a]"), parse = T, 
  ...
  scale_linetype_manual(values = c(1,2), 
                        labels = c(expression(rho[0]), 
                                   expression(rho[1])), name="") +
...
p1 <- ggplot(data.frame(x = c(-10, 10)), aes(x)) +
  ...
  geom_label(aes(x = 0.2, y = 0.12, label = "beta[b]"), parse = T, 
             fill = "darkgreen", alpha = 0.4) +
  geom_label(aes(x = 2.6, y = 0.08, label = "alpha[b]"), parse = T, 
             fill = "orange", alpha = 0.4) +

  geom_text(aes(x=1, y=-0.04, label = "x[0]"), parse = T) +
  geom_text(aes(x=2, y=-0.04, label = "x[1]"), parse = T) +
  
  stat_function(fun = dnorm, geom = "line", aes(linetype = "c"), 
                xlim = c(0,0)) +
...
p2 <- ggplot(data.frame(x = c(-10, 10)), aes(x)) +
  # Midpoint
  stat_function(fun = dnorm, args = list(mean = 1.25), geom = "line", 
                linetype=3) +
  ...
  geom_label(aes(x = -0.9, y = 0.12, label = "gamma[L]"), parse = T, 
             fill = "red", alpha = 0.4) +
  geom_label(aes(x = 3.4, y = 0.12, label = "gamma[U]"), parse = T, 
             fill = "blue", alpha = 0.4) +
